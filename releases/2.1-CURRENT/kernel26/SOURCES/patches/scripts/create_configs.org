...
sub config_xbox {
    foreach (@enable_xbox_options) {
	$values{$_} = "y";
    }

    foreach (@disable_xbox_options) {
	$values{$_} = "n";
    }

    foreach (@module_xbox_options) {
	$values{$_} = "m";
    }
    $to_add{FB_XBOX}="y";
    $to_add{HOTPLUG_PCI_SHPC_PHPRM_LEGACY}="n";
}
...
sub config_nproc_ppc {
    my ($nproc) = @_;
    
    config_nproc($nproc);

    if ($nproc eq "smp" ) {
	$to_add{IRQ_ALL_CPUS} = "y";
        $to_add{CPUSETS} = "n";
    } else {
	# nothing by now
    }
...
sub config_nproc_ppc64 {
    ...
    $values{PPC_PSERIES} = "n";
}

sub config_nproc_power5 {
    my ($nproc) = @_;
    
    config_nproc($nproc);

    if ($nproc ne "smp" ) {
	print STDERR "Internal error, Power5 kernels shall only be SMP\n";
	exit 1;
    }

    $to_add{CPUSETS} = "y";
    $to_add{DISCONTIGMEM} = "y";
    $to_add{NUMA} = "y";
    $to_add{SCHED_SMT} = "y";
    $values{ALTIVEC} = "n";
    $values{PPC_PMAC} = "n";
    ...
    $values{POWER5_ONLY} = "y";

    # disable Macintosh drivers
    $values{ADB} = "n";
    $values{PMAC_SMU} = "n";
    $values{THERM_PM72} = "n";
    $values{SERIAL_PMACZILOG} = "n";
    $values{I2C_KEYWEST} = "n";
}

sub config_nproc_ia64 {
    my ($nproc) = @_;
...
sub config_i386 {
    ...
    config_xbox_x86() if $name eq "xbox";
...
sub config_ppc {
    ...
    config_nproc_ppc($nproc);
    config_ppc_highmem($mem);
...
sub config_ppc64 {
    ...
    if ($name eq "power5") {
	config_nproc_power5($nproc);
    } else {
	config_nproc_ppc64($nproc);
    }
}

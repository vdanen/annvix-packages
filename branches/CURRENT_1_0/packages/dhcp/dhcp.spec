...
./configure --copts "$RPM_OPT_FLAGS $PTR64_FLAGS"
%make
...
# use the original omapi.3 because it is preformatted better
# than the one generated by nroff in the makefile:
#rm -f %{buildroot}%{_mandir}/man3/omapi.3*
 
# put the preformatted page in cat3:
#install -d -m 755 %{buildroot}/%{_catdir}/cat{1,3,5,8}
#for i in 1 3 5 8; do 
#	install -m 644 `find $RPM_BUILD_DIR -type f |grep cat$i` %{buildroot}/%{_catdir}/cat$i
#done

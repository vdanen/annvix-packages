...
static const char identification[] = "$Identification: 27 $";
		  ...
		  int			wheel)
{
	...
	/* Simulate buttons 4 and 5 for Graphire wheel and Intuos2 2D mouse wheel */
	if ((((common->wcmFlags & GRAPHIRE_FLAG) && !is_stylus) || (common->wcmFlags & TWODMOUSE_FLAG))
		&& (wheel != 0)) {
	    int fake_button;
...
xf86WcmReadInput(LocalDevicePtr         local)
{
	...
	*/
	...
	y = 0;
		...
		DBG(8, ErrorF("xf86WcmReadInput %s side is_proximity %d\n",
		common->wcmStylusSide ? "stylus" : "eraser", is_proximity));
		...
		/* 2D mouse */
		    ...
		    ds->buttons = ((common->wcmData[8] & 0x1d) >> 2);
		    ds->wheel = ((common->wcmData[8] & 0x02) >> 1) -
		    		 (common->wcmData[8] & 0x01 );
		    have_data = 1;
		}
		/* Lens cursor */
		else {
...
#define MOD_BUTTONS(bit, value) \
	...
	case EV_REL:
	    switch (event->code) {
	    case REL_WHEEL:
		common->wcmFlags &= TWODMOUSE_FLAG;
		wheel = event->value;
		break;
	    default:
